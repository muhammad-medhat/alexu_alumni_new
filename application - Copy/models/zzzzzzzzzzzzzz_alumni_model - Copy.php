...
//      $year_r = $this->input->post('year-r');
   ...
   $query = $this->db->count_all_results();
    ...
    //$debug = "<div class='query'>".$this->db->last_query() .'</div>';
  ...
  function get_alumni($_alumni_id)
  {
      ...
      $this->db->select("g.*, ac.$academic_degree_field, u.*, f.$faculty_field, d.$degree_field", true);
      $this->db->from("$this->alumni_data_table g");
      $this->db->join("$this->alumni_main_table m", 'g.alumni_id=m.id', 'left');
      $this->db->join("$this->users_table u", 'm.user_id=u.id' , 'left');
      $this->db->join("$this->faculties_table f", 'f.id=g.fid', 'left');
...
/*    $q = "insert into " .$this->use`rs_table ."(name, email, username, password,  usertype) 
 ...
 //   $this->add_alumni_data($alumni_id, 0);  
